...
# ── STYLE ──────────────────────────────────────────────────────────────────────
# BG  = #0a1f0a  (dark green background)
# BG2 = #0f2d0f  (slightly lighter dark green for cards and hints)
...
<style>
    ...
    [data-testid="stMetricLabel"]  { font-size: 13px !important; color: #86efac !important; font-weight: 600 !important; }
    [data-testid="stMetricValue"]  { font-size: 28px !important; font-weight: 700 !important; color: #e8eef6 !important; }
    [data-testid="stMetricDelta"]  { font-size: 12px !important; }
    .block-title {
        font-size: 16px; font-weight: 700; color: #e8eef6;
        padding: 6px 12px; background: #0f2d0f;
        border-left: 4px solid #00d4aa;
        border-radius: 4px; margin-bottom: 8px; margin-top: 4px;
    }
    .hint {
        font-size: 12px; color: #86efac; margin-top: 4px;
        padding: 5px 10px; background: #0f2d0f;
        border-radius: 4px; margin-bottom: 4px;
    }
    hr { margin: 0.4rem 0 !important; border-color: #1a3d1a !important; }
    [data-testid="stSelectbox"] label { color: #86efac !important; }
</style>
...
with k1:
    st.metric("Total Loans", f"{total:,}", help="Total loans in current filtered view.")
with k2:
...
with col1:
    ...
    fig1.update_layout(**C, title="Default Rate by Loan Purpose",
                       xaxis=dict(title="Default Rate %", gridcolor="#1a3d1a", range=[0, 40]),
                       yaxis=dict(title=""), height=300)
    st.plotly_chart(fig1, use_container_width=True)
    st.markdown('<div class="hint">Green = healthy &nbsp;|&nbsp; Yellow = watch &nbsp;|&nbsp; Red = critical</div>', unsafe_allow_html=True)
...
with col2:
    ...
    fig2.update_layout(**C, title="Loan Risk Bubble — DTI vs LTI",
        xaxis=dict(title="DTI Level →  Higher = More Risk", tickfont=dict(size=12)),
        yaxis=dict(title="LTI Level ↑  Higher = More Risk", tickfont=dict(size=12)),
        height=320, coloraxis_colorbar=dict(title="Default %"))
...
with col3:
    ...
    fig3.update_layout(**C, title="Default Rate Drops as Credit Score Rises",
        ...
        yaxis=dict(title="Default Rate %", gridcolor="#1a3d1a", range=[0, 50]), height=320)
    st.plotly_chart(fig3, use_container_width=True)
    st.markdown('<div class="hint">Line drops sharply after 670 — your approval threshold.</div>', unsafe_allow_html=True)
...
with col4:
    ...
    fig4 = go.Figure(go.Bar(
        ...
        textposition="outside", textfont=dict(size=14, color="white"), width=0.5,
    ))
    ...
    fig4.update_layout(**C, title="Default Rate by Employment Type",
        ...
        yaxis=dict(title="Default Rate %", gridcolor="#1a3d1a", range=[0, 55]), height=320)
...
with col5:
    ...
    fig5.update_layout(**C, title="Co-Signer Cuts Default Rate — by Loan Purpose",
        ...
        yaxis=dict(title="Default Rate %", gridcolor="#1a3d1a", range=[0, 40]),
        legend=dict(bgcolor="#0a1f0a", font=dict(size=12)), height=340)
...
with col6:
    ...
    fig6.add_trace(go.Scatter(
        ...
        mode="lines", line=dict(color="#1a3d1a", width=3), showlegend=False,
    ))
    fig6.add_trace(go.Scatter(
        x=edu_stats["Education"].astype(str), y=edu_stats["DefaultRate"],
        mode="markers+text",
    ...
    fig6.update_layout(**C, title="Default Rate by Education Level",
        xaxis=dict(title="", tickfont=dict(size=13)),
        yaxis=dict(title="Default Rate %", gridcolor="#1a3d1a", range=[0, 45]), height=340)
