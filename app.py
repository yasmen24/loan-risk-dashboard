...
# ── STYLE — FULLY FORCED, IMMUNE TO LIGHT/DARK MODE ───────────────────────────
# Every color is explicitly set. Streamlit cannot override these.
st.markdown("""
<style>
    /* ── FORCE BACKGROUND ON EVERYTHING ── */
    html, body { background-color: #0e1117 !important; color: #e8eef6 !important; }
    ...
    .main, .main > div               { background-color: #0e1117 !important; }
    ...
    *, p, span, div, label {
    ...
    [data-testid="stMetricDeltaIcon"] { color: inherit !important; }
    ...
    div[data-baseweb="select"] > div  { background-color: #1a202c !important; color: #e8eef6 !important; }
    div[data-baseweb="popover"] *     { background-color: #1a202c !important; color: #e8eef6 !important; }

    /* ── DIVIDER ── */
    hr { border-color: #2d3748 !important; margin: 0.4rem 0 !important; }

    /* ── SECTION TITLE ── */
    .block-title {
        font-size: 16px !important; font-weight: 700 !important;
        color: #e8eef6 !important; padding: 6px 12px !important;
        background: #1a202c !important; border-left: 4px solid #00d4aa !important;
        border-radius: 4px !important; margin-bottom: 8px !important; margin-top: 4px !important;
    }

    /* ── HINT BOX ── */
    .hint {
        font-size: 12px !important; color: #a0aec0 !important;
        margin-top: 4px !important; padding: 5px 10px !important;
        background: #1a202c !important; border-radius: 4px !important;
    ...
    /* ── SCROLLBAR ── */
    ::-webkit-scrollbar              { width: 6px; background: #0e1117; }
...
with f1:
    purpose_filter = st.selectbox("Loan Purpose",
...
with f2:
    emp_filter = st.selectbox("Employment Type",
        ...
        help="Filter by borrower job status. Unemployed = highest default rate.")
with f3:
    risk_filter = st.selectbox("Risk Tier", ["All", "Low", "Medium", "High"],
        help="Low = safe (0–2 pts). Medium = watch (3–5 pts). High = danger (6+ pts).")
...
with k1:
    st.metric("Total Loans", f"{total:,}",
              help="Total loans in current filtered view.")
with k2:
...
with k5:
    st.metric("Avg LTI Ratio", f"{avg_lti:.2f}",
              ...
              help="Loan-to-Income. Above 4 = very stretched.")
with k6:
    st.metric("High Risk Loans", f"{high_pct:.1f}%",
              ...
              help="Above 25% = serious portfolio risk.")
...
with col1:
    ...
    bar_colors = ["#ff4560" if r > 20 else "#ffc145" if r > 12 else "#00d4aa"
                  for r in purpose_stats["DefaultRate"]]

    fig1 = go.Figure(go.Bar(
        x=purpose_stats["DefaultRate"], y=purpose_stats["LoanPurpose"],
        orientation="h", marker_color=bar_colors,
        text=[f"{r:.1f}%  ({c:,} loans)" for r, c in zip(purpose_stats["DefaultRate"], purpose_stats["Count"])],
        textposition="outside", textfont=dict(size=13, color="#e8eef6"),
    ))
    ...
    fig1.update_layout(**C, title="Default Rate by Loan Purpose",
        xaxis=dict(title="Default Rate %", gridcolor="#1e2736", range=[0, 40], color="#e8eef6"),
        yaxis=dict(title="", color="#e8eef6"), height=300)
    st.plotly_chart(fig1, use_container_width=True)
    st.markdown('<div class="hint">Green = healthy &nbsp;|&nbsp; Yellow = watch &nbsp;|&nbsp; Red = critical &nbsp;|&nbsp; Dashed lines = thresholds</div>', unsafe_allow_html=True)

with col2:
    fc = filtered.copy()
    fc["DTIBucket"] = pd.cut(fc["DTIRatio"], bins=[0, 0.2, 0.35, 0.5, 0.65, 0.8, 1.0],
        labels=["Very Low", "Low", "Moderate", "High", "Very High", "Extreme"])
    fc["LTIBucket"] = pd.cut(fc["LTIRatio"].clip(upper=10), bins=[0, 1, 2, 4, 6, 10],
        labels=["Very Low", "Low", "Moderate", "High", "Extreme"])

    bubble_data = fc.groupby(["DTIBucket", "LTIBucket"], observed=True).agg(
        TotalLoans=("Default", "count"), Defaults=("Default", "sum")
    ).reset_index()
    bubble_data["DefaultRate"] = (bubble_data["Defaults"] / bubble_data["TotalLoans"] * 100).round(1)

    fig2 = px.scatter(bubble_data, x="DTIBucket", y="LTIBucket",
        ...
        size_max=40)
    fig2.update_layout(**C, title="Loan Risk Bubble — DTI vs LTI",
        xaxis=dict(title="DTI Level →  Higher = More Risk", tickfont=dict(size=12, color="#e8eef6"), color="#e8eef6"),
        yaxis=dict(title="LTI Level ↑  Higher = More Risk", tickfont=dict(size=12, color="#e8eef6"), color="#e8eef6"),
        height=320, coloraxis_colorbar=dict(title="Default %", tickfont=dict(color="#e8eef6"), title_font=dict(color="#e8eef6")))
    st.plotly_chart(fig2, use_container_width=True)
    st.markdown('<div class="hint">Bubble size = number of loans &nbsp;|&nbsp; Color = default rate &nbsp;|&nbsp; Top-right = highest risk</div>', unsafe_allow_html=True)
...
with col3:
    fc2 = filtered.copy()
    fc2["CreditBand"] = pd.cut(fc2["CreditScore"], bins=[300, 500, 580, 670, 740, 851],
    ...
    dot_colors = ["#ff4560" if r > 20 else "#ffc145" if r > 12 else "#00d4aa"
                  for r in credit_stats["DefaultRate"]]

    fig3 = go.Figure()
    fig3.add_trace(go.Scatter(
        x=credit_stats["CreditBand"].astype(str), y=credit_stats["DefaultRate"],
        ...
        textposition="top center", textfont=dict(size=13, color="#e8eef6"),
    ))
    ...
    fig3.update_layout(**C, title="Default Rate Drops as Credit Score Rises",
        xaxis=dict(title="Credit Score Band", tickfont=dict(size=12, color="#e8eef6"), color="#e8eef6"),
        yaxis=dict(title="Default Rate %", gridcolor="#1e2736", range=[0, 50], color="#e8eef6"),
        height=320)
    st.plotly_chart(fig3, use_container_width=True)
    st.markdown('<div class="hint">Line drops sharply after 670 — your approval threshold. Below it = high risk.</div>', unsafe_allow_html=True)
...
with col4:
    ...
    emp_colors = ["#ff4560" if r > 20 else "#ffc145" if r > 12 else "#00d4aa"
                  for r in emp_stats["DefaultRate"]]

    fig4 = go.Figure(go.Bar(
        x=emp_stats["EmploymentType"], y=emp_stats["DefaultRate"],
        marker_color=emp_colors,
        text=emp_stats["DefaultRate"].apply(lambda x: f"{x:.1f}%"),
        textposition="outside", textfont=dict(size=14, color="#e8eef6"), width=0.5,
    ))
    ...
    fig4.update_layout(**C, title="Default Rate by Employment Type",
        xaxis=dict(title="", tickfont=dict(size=13, color="#e8eef6"), color="#e8eef6"),
        yaxis=dict(title="Default Rate %", gridcolor="#1e2736", range=[0, 55], color="#e8eef6"),
        height=320)
...
with col5:
    ...
    fig5.add_trace(go.Bar(
        ...
        textposition="outside", textfont=dict(size=12, color="#e8eef6"),
    ))
    fig5.add_trace(go.Bar(
    ...
    fig5.update_layout(**C, title="Co-Signer Cuts Default Rate — by Loan Purpose",
        barmode="group",
        xaxis=dict(title="", tickfont=dict(size=12, color="#e8eef6"), color="#e8eef6"),
        yaxis=dict(title="Default Rate %", gridcolor="#1e2736", range=[0, 40], color="#e8eef6"),
        legend=dict(bgcolor="#1a202c", font=dict(size=12, color="#e8eef6"),
                    bordercolor="#2d3748", borderwidth=1),
        height=340)
    st.plotly_chart(fig5, use_container_width=True)
    st.markdown('<div class="hint">Green bars always lower than red — co-signer reduces risk in every loan type.</div>', unsafe_allow_html=True)
...
with col6:
    ...
    fig6.update_layout(**C, title="Default Rate by Education Level",
        xaxis=dict(title="", tickfont=dict(size=13, color="#e8eef6"), color="#e8eef6"),
        yaxis=dict(title="Default Rate %", gridcolor="#1e2736", range=[0, 45], color="#e8eef6"),
        height=340)
    st.plotly_chart(fig6, use_container_width=True)
    st.markdown('<div class="hint">Each dot = one education group &nbsp;|&nbsp; Number below = loan count.</div>', unsafe_allow_html=True)
